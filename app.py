...
def read_csv_fast(path: Path, progress_cb=None, encoding_try=("utf-8", "utf-8-sig", "latin1"), import_mode: str = "string", usecols=None) -> pd.DataFrame:
    ...
    with open(path, "rb") as f: sample = f.read(65536)
    try: sample_txt = sample.decode("utf-8", errors="ignore")
    except Exception: sample_txt = sample.decode("latin1", errors="ignore")
        ...
        try:
            with warnings.catch_warnings():
                ...
                for ch in reader:
                    ...
                    if progress_cb: progress_cb(approx / total)
...
def _parse_number_series(s: pd.Series, decimal_sep: str | None = None, thousands_sep: str | None = None) -> pd.Series:
    if pd.api.types.is_numeric_dtype(s.dtype): return s.astype("Float64")
    x = s.astype(STRING_DTYPE).str.strip()
    if thousands_sep: x = x.str.replace(thousands_sep, "", regex=False)
    if decimal_sep and decimal_sep != ".": x = x.str.replace(decimal_sep, ".", regex=False)
...
def _apply_format(file_name: str, df: pd.DataFrame, formats: dict) -> pd.DataFrame:
    ...
    for col, spec in formats.items():
        ...
        if t in ("date", "datetime"):
            ...
            if mode == "fmt" and fmt: out[col] = pd.to_datetime(out[col], format=fmt, errors="coerce")
            else: out[col] = pd.to_datetime(out[col], errors="coerce")
    return out

def _filter_mask_single(df: pd.DataFrame, col: str, kind: str, op: str, v1, v2):
    if col not in df.columns: return pd.Series([True] * len(df), index=df.index)
    s = df[col]
    if kind == "date":
        dt = pd.to_datetime(s, errors="coerce")
        d1 = pd.to_datetime(v1, errors="coerce")
        d2 = pd.to_datetime(v2, errors="coerce") if v2 else None
        if op == "after": return dt > d1
        if op == "before": return dt < d1
        if op == "eq": return dt == d1
        if op == "neq": return dt != d1
    ...
    if kind == "number":
        ...
        if op == "eq": return num == n1
        if op == "neq": return num != n1
    ...
    if op == "eq": return txt == v1s
    if op == "neq": return txt != v1s
    if op == "contains": return txt.str.contains(re.escape(v1s), case=False, na=False)
    if op == "not_contains": return ~txt.str.contains(re.escape(v1s), case=False, na=False)
    if op == "startswith": return txt.str.startswith(v1s, na=False)
    if op in ("finisce con", "endswith"): return txt.str.endswith(v1s, na=False)
    if op == "is_empty": return txt.str.len().fillna(0) == 0
    if op == "not_empty": return txt.str.len().fillna(0) > 0
    return pd.Series([True] * len(df), index=df.index)
...
def _apply_dedupe(df: pd.DataFrame, params: dict) -> pd.DataFrame:
    if params["col"] not in df.columns: return df
...
def _safe_merge(left: pd.DataFrame, right: pd.DataFrame, how: str, left_on: str, right_on: str, collision: str = "suffix", suffix_left: str = "_A", suffix_right: str = "_B"):
    ...
    if not dup: return pd.merge(left, right, how=how, left_on=left_on, right_on=right_on, copy=False)
    if collision == "keep_left": return pd.merge(left, right.drop(columns=list(dup), errors="ignore"), how=how, left_on=left_on, right_on=right_on, copy=False)
    if collision == "keep_right": return pd.merge(left.drop(columns=list(dup), errors="ignore"), right, how=how, left_on=left_on, right_on=right_on, copy=False)
    return pd.merge(left, right.rename(columns={c: f"{c}{suffix_right}" for c in dup}), how=how, left_on=left_on, right_on=right_on, copy=False)
...
def ifs_chunked_rowmatch(file_target: str, df_target: pd.DataFrame, lookup_file: str, lookup_df: pd.DataFrame, params: dict, progress_cb=None, chunk_rows: int = 300_000) -> pd.DataFrame:
    fun, match_a_col, match_b_col, val_col = params["metric"], params["match_a_col"], params["match_b_col"], params.get("value_col", "")
    if match_a_col not in df_target.columns: raise ValueError(f"Colonna A '{match_a_col}' non trovata.")
    if match_b_col not in lookup_df.columns: raise ValueError(f"Colonna B '{match_b_col}' non trovata.")

    b = lookup_df
    for c in params.get("conds", []): b = _apply_filter(b, c)
    bkey = b[match_b_col].astype(STRING_DTYPE).fillna("")

    if fun == "countifs":
        agg_map = bkey.value_counts(dropna=False)
    else:
        if declared_type_for_col(lookup_file, val_col) == "text": raise ValueError(f"Colonna valore formattata come Testo.")
        if val_col not in b.columns: raise ValueError("Colonna valore non trovata in B.")
        grp = _parse_number_series(b[val_col]).groupby(bkey, dropna=False)
        agg_map = grp.sum(min_count=1) if fun == "sumifs" else grp.mean()

    n, out = len(df_target), df_target.copy()
    outcol = params["outcol"]
    if outcol not in out.columns: out[outcol] = np.nan
    ...
    for start in range(0, n, chunk_rows):
        ...
        if fun == "countifs": res = res.fillna(0).astype("int64")
        elif fun == "sumifs": res = res.fillna(0)
        out.iloc[start:end, col_idx] = res.to_numpy()
        if progress_cb: progress_cb(end / max(1, n))
    return out
...
def _apply_calc_cols(file_name: str, df: pd.DataFrame, params: dict) -> pd.DataFrame:
    ...
    if base_col not in df.columns: raise ValueError(f"Colonna base '{base_col}' non trovata.")
    if declared_type_for_col(file_name, base_col) == "text": raise ValueError("Operazioni non consentite su Testo.")

    res = _parse_number_series(df[base_col]).fillna(0.0).astype("Float64")
    for step in ops:
        op, c = step["op"], step["col"]
        if c not in df.columns: raise ValueError(f"Colonna '{c}' non trovata.")
        nxt = _parse_number_series(df[c]).fillna(0.0).astype("Float64")
        if op == "add": res = res + nxt
        elif op == "sub": res = res - nxt
        elif op == "mul": res = res * nxt
        elif op == "div": res = res / nxt
...
def apply_step(step: dict, df_in: pd.DataFrame, get_df_fn, current_file: str | None = None, progress_cb=None):
    ...
    if stype == "format": return _apply_format(current_file, df_in, p.get("formats", {}))
    if stype == "filter": return _apply_filter(df_in, p)
    if stype == "filter_multi": return _apply_filter_multi(df_in, p)
    if stype == "dedupe": return _apply_dedupe(df_in, p)
    if stype == "calc": return _apply_calc_cols(current_file, df_in, p)
    if stype == "merge":
        right_file = p["right_file"]
        key = (current_file, right_file)
        if key in _recompute_stack: raise ValueError("Loop rilevato nel merge.")
        _recompute_stack.add(key)
        try: right_df = get_df_fn(right_file)
        finally: _recompute_stack.discard(key)
    ...
    if stype == "ifs":
        ...
        if key in _recompute_stack: raise ValueError("Loop rilevato in IFS.")
        _recompute_stack.add(key)
        try: lookup_df = get_df_fn(lookup_file)
        finally: _recompute_stack.discard(key)
...
def get_df(fname: str, progress_cb=None) -> pd.DataFrame:
    ...
    if "smart_cache" not in rec: rec["smart_cache"] = {"step_idx": -1, "df": base}
...
def compute_metric(file_name: str, df: pd.DataFrame, col: str, metric: str, q: float | None = None):
    if col not in df.columns: raise ValueError("Colonna non trovata.")
    ...
    s = df[col]
    if metric == "counta": return int(s.notna().sum())
    if metric == "count": return int(pd.to_numeric(s, errors="coerce").notna().sum())
    vals = _parse_number_series(s)
    if metric == "sum": return float(vals.sum(skipna=True))
    if metric == "average": return float(vals.mean(skipna=True))
    if metric == "median": return float(vals.median(skipna=True))
    if metric == "quartile": return float(vals.quantile(q or 0.25))
    if metric == "percentile": return float(vals.quantile(q or 0.50))
...
def step_label(st_dict: dict) -> str:
    ...
    if t == "format": return f"Formato: {', '.join([f'{c}:{v['type']}' for c, v in p.get('formats', {}).items()])}"
    if t == "filter": return f"Filtro: {p['col']} {'tra '+str(p['v1'])+' e '+str(p['v2']) if p['op']=='between' else p['op']+' '+str(p['v1'])}"
    if t == "filter_multi": return f"Filtro multiplo ({p.get('logic','AND')}) con {len(p.get('conds',[]))} condiz."
    if t == "merge": return f"MERGE ({p['how'].upper()}) con '{p['right_file']}' su '{p['left_on']}'='{p['right_on']}'"
    if t == "ifs": return f"{p['metric'].upper()} → '{p['outcol']}' (B={p['lookup_file']})"
    if t == "calc":
        if "col_left" in p: return f"CALC: {p['outcol']} = [{p['col_left']}] {p['op']} [{p['col_right']}]"
        seq = [f"[{p['base_col']}]"] + [f"{s['op']} [{s['col']}]" for s in p.get("ops", [])]
        return f"CALC: {p['outcol']} = {' '.join(seq)}"
    if t == "dedupe": return f"Deduplica su: {p.get('col','')}"
    return t
...
with t_files:
    ...
    import_mode = st.radio("Modalità Importazione:", ["smart", "string"], format_func=lambda x: "SMART (Riconosce Date/Numeri - Veloce)" if x=="smart" else "TUTTO STRING (Testo - Massima Stabilità)")
    ...
    if uploaded_files:
        if st.button("Carica File Selezionati", type="primary"):
            ...
            for idx, uf in enumerate(uploaded_files):
                # Salvataggio temporaneo per usare il motore chunked basato su Path
                temp_path = Path(st.session_state.temp_dir) / uf.name
                with open(temp_path, "wb") as f:
                    f.write(uf.getbuffer())
                
                try:
                    df = read_file(temp_path, progress_cb=lambda f: progress_bar.progress((idx + f)/len(uploaded_files), text=f"Caricamento {uf.name} ({f*100:.0f}%)"), import_mode=import_mode)
        ...
        for fname in files_in_mem:
            with st.expander(f"📄 {fname} ({len(st.session_state.STATE['files'][fname]['df_base']):,} righe base)", expanded=False):
                ...
                if not steps:
                    ...
                    try: get_df(fname) # aggiorna cache
...
# Helper funzioni dropdown dinamiche
def get_cols(fname):
    if not fname: return []
    try: return list(get_df(fname).columns)
    ...
    if files_in_mem:
        ...
        with col1:
            ...
            with st.form("fmt_form"):
                ...
                # Le visibilità dinamiche si gestiscono con if in Streamlit
                date_input = None
                ...
                if st.form_submit_button("✅ Applica Formato"):
                    spec = {"type": fmt_type}
        ...
        with col2:
            ...
            if prev_file:
                df_prev = get_df(prev_file).head(100) # mostriamo le prime 100 per UI veloce
...
with t_flt:
    if files_in_mem:
        ...
        with st.form("single_filter_form"):
            ...
            flt_kind = c3.selectbox("Tipo dato:", ["number", "text", "date"])
            
            c4, c5 = st.columns([1, 2])
            ops = OPS_NUMBER if flt_kind == "number" else (OPS_DATE if flt_kind == "date" else OPS_TEXT)
            flt_op = c4.selectbox("Operatore:", [v for k,v in ops], format_func=lambda x: dict(ops).get(next(k for k,v in ops if v==x), x))
            
            if flt_kind == "date":
                v1 = c5.date_input("Valore 1")
                v2 = c5.date_input("Valore 2") if flt_op == "between" else None
            else:
                v1 = c5.text_input("Valore 1")
                v2 = c5.text_input("Valore 2") if flt_op == "between" else None

            if st.form_submit_button("✅ Applica Filtro Singolo"):
                val1 = v1.isoformat() if flt_kind == "date" else v1
                val2 = v2.isoformat() if (flt_kind == "date" and v2) else v2
                step = {"type":"filter", "created_at": datetime.now().isoformat(), "params":{"col": flt_col, "kind": flt_kind, "op": flt_op, "v1": val1, "v2": val2}}
        ...
        with st.form("dedupe_form"):
            ...
            if st.form_submit_button("🧹 Rimuovi Duplicati"):
...
with t_merge:
    if files_in_mem:
        ...
        merge_mode = st.radio("Destinazione:", ["existing", "new"], format_func=lambda x: "Mergia su file esistente (A)" if x=="existing" else "Crea NUOVO file in memoria")
        new_name = st.text_input("Nome nuovo file:", "merged_1") if merge_mode == "new" else None
        ...
        c5, c6 = st.columns(2)
        m_how = c5.selectbox("Tipo Join:", ["left", "right", "inner", "outer"])
        m_coll = c6.selectbox("Collisioni nomi:", ["suffix", "keep_left", "keep_right"], format_func=lambda x: "Aggiungi _B automatico" if x=="suffix" else x)
        
        if st.button("🔗 Esegui Merge", type="primary"):
            if merge_mode == "existing":
                step = {"type": "merge", "created_at": datetime.now().isoformat(), "params": {"right_file": m_B, "how": m_how, "left_on": m_on_A, "right_on": m_on_B, "collision": m_coll}}
                _base_add_step(m_A, step)
                st.success("Merge completato!")
            else:
                create_new_file_from_merge(sanitize_name(new_name), m_A, m_B, m_how, m_on_A, m_on_B, m_coll)
                st.success(f"Nuovo file {sanitize_name(new_name)} creato!")
            st.rerun()
...
with t_calc:
    if files_in_mem:
        ...
        with st.container(border=True):
            ...
            add_op = cc1.selectbox("Op:", ["add", "sub", "mul", "div"], format_func=lambda x: {"add":"+ (Somma)","sub":"- (Sottrai)","mul":"* (Moltiplica)","div":"/ (Dividi)"}[x])
            ...
            if st.session_state.calc_ops_list:
                formula = f"[{calc_base}]"
                for i, op in enumerate(st.session_state.calc_ops_list):
        ...
        calc_out = st.text_input("Nome nuova colonna:", "calc_row_1")
        if st.button("✅ Esegui Calcolo", type="primary"):
            step = {"type": "calc", "created_at": datetime.now().isoformat(), "params": {"base_col": calc_base, "ops": st.session_state.calc_ops_list, "outcol": calc_out}}
        ...
        with st.form("ifs_form"):
            ...
            c5, c6 = st.columns(2)
            metric = c5.selectbox("Funzione:", ["countifs", "sumifs", "averageifs"])
            val_col = c6.selectbox("Colonna Valore (in B):", [""] + get_cols(ifs_B))
            
            ifs_out = st.text_input("Nome nuova colonna in A:", "ifs_result_1")
            
            if st.form_submit_button("✅ Esegui IFS"):
                if metric in ["sumifs", "averageifs"] and not val_col:
                    st.error("Seleziona una colonna valore per SUM/AVERAGE.")
                else:
                    step = {"type": "ifs", "created_at": datetime.now().isoformat(), "params": {"lookup_file": ifs_B, "match_a_col": ifs_match_a, "match_b_col": ifs_match_b, "metric": metric, "value_col": val_col, "outcol": ifs_out, "conds": st.session_state.run_conds}}
...
with t_metr:
    if files_in_mem:
        ...
        c3, c4 = st.columns(2)
        met_kind = c3.selectbox("Metrica:", ["sum", "count", "counta", "average", "median", "quartile", "percentile"])
        q_val = c4.text_input("Q/Percentile (es. 0.25):", "0.25") if met_kind in ["quartile", "percentile"] else None
...
with t_exp:
    if files_in_mem:
        ...
        exp_name = st.text_input("Nome file output:", f"export_{export_f}")
        
        # Buffer in memory per non scrivere su disco del server prima del download
        if st.button("Genera File CSV"):
            df_out = get_df(export_f)
            
            # Utilizziamo PyArrow se disponibile (molto più veloce a convertire in CSV)
            if HAS_PYARROW:
